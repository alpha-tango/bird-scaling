...
set -x
infile='/Volumes/Expansion/ebird/ebd_relOct-2025_parquet/ebd_relOct-2025_2023.txt'
sql=$(cat << END
...
COPY (
    SELECT * from read_csv(
        '/dev/stdin',        delim='\t',
        header=false, ignore_errors=true,
...
) to '/Volumes/Expansion/ebird/ebd_relOct-2025_parquet/ebd_relOct-2025_2023.parquet' (FORMAT PARQUET);
END
)
time awk -F'\t' 'BEGIN {OFS="\t"} {print $0}' $infile | duckdb :memory: "$sql"